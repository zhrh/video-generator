...
bool GetGenNames(const std::string &savepath, const unsigned int nameid,const int type_num, std::vector<std::string> &gen_names)
{
	...
	{
		sprintf(nameid_str,"%u",nameid + i * step);	// 这里一定要使用%u代表无符号, %d代表有符号
		save_name.assign(savepath + "/" + nameid_str + ".avi");
...
//	}
	...
	const int kMaxFrameCount = 7200;	// 8 * 15 * 15(fps)	// 为测试速度, 控制视频长度
